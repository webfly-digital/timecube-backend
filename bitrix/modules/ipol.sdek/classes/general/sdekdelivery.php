...
class CDeliverySDEK extends sdekHelper
{
    ...
    static function Init()
    {
        return [
            ...
            // Handler methods
            "DBGETSETTINGS" => ["CDeliverySDEK", "GetSettings"],
            "DBSETSETTINGS" => ["CDeliverySDEK", "SetSettings"],
            "GETCONFIG" => ["CDeliverySDEK", "GetConfig"],

            "COMPABILITY" => ["CDeliverySDEK", "Compability"],
            "CALCULATOR" => ["CDeliverySDEK", "Calculate"],
    ...
    static function GetConfig()
    {
        ...
        $arAccs = [false => GetMessage("IPOLSDEK_DELCONFIG_DEFAULT")];
        ...
        $arSenders = [false => GetMessage('IPOLSDEK_DELCONFIG_DEFAULT'), $city['SDEK_ID'] => $city['NAME'] . " (" . GetMessage('IPOLSDEK_DELCONFIG_BACIS') . ")"];
        ...
        $arCountries = [false => \Ipolh\SDEK\Bitrix\Tools::getMessage('LBL_ALL')];
    ...
    static function Compability($arOrder, $arConfig)
    {
        ...
        if ($arCity) {
                ...
                if (
                    ...
                    foreach ($countries as $key => $country) {
                        if (self::$sdekCityCntr == $country) {
                            $activeCountries = (is_array($arConfig['COUNTRIES']['VALUE'])) ? $arConfig['COUNTRIES']['VALUE'] : [$arConfig['COUNTRIES']['VALUE']];
            ...
            if (!(array_key_exists('isdek_action', $_POST) && $_POST['isdek_action'])
                ...
                if (!self::$preSet && $arItems = sdekShipmentCollection::formation($arOrder)) {
                    ...
                    foreach (GetModuleEvents(self::$MODULE_ID, "onBeforeShipment", true) as $arEvent)
                        ExecuteModuleEventEx($arEvent, [&$order, $arItems]);
                ...
                foreach ($arKeys as $key => $profile) {
                    if (array_key_exists($profile, $pickupProfilesAvailable) && !$pickupProfilesAvailable[$profile]) {
                        unset($arKeys[$key]);
                    }
                }
            ...
            foreach (GetModuleEvents(self::$MODULE_ID, "onCompabilityBefore", true) as $arEvent)
                $ifPrevent = ExecuteModuleEventEx($arEvent, [$arOrder, $arConfig, $arKeys]);
    ...
    static function Calculate($profile, $arConfig, $arOrder, $STEP, $TEMP = false)
    {
                ...
                if ($curProfile['RESULT'] == "OK") {
                    ...
                    */
                    ...
                    if (\Ipolh\SDEK\option::get('mindEnsure') == 'Y') {
                        $priceByCurrency = CCurrencyRates::ConvertCurrency($arOrder['PRICE'], $arOrder['CURRENCY'], 'RUB');
                        $insurance = $priceByCurrency * floatval(\Ipolh\SDEK\option::get('ensureProc')) / 100;
                        if (\Ipolh\SDEK\option::get('mindNDSEnsure') == 'Y') {
                            ...
                            $insurance += $insurance * $vatNDS / 100;
                        }
                        $curProfile['PRICE'] += $insurance;
                    }
        ...
        foreach (GetModuleEvents(self::$MODULE_ID, "onCalculate", true) as $arEvent)
            ExecuteModuleEventEx($arEvent, [&$arReturn, $profile, $arConfig, $arOrder]);
        ...
        \Ipolh\SDEK\Bitrix\Admin\Logger::calculate(['Profile' => $profile, 'Result' => $arReturn]);
        \Ipolh\SDEK\Bitrix\Admin\Logger::shipments(['Profile' => $profile, 'Shipments' => sdekShipmentCollection::$shipments]);
    ...
    public static function getDeliverySender($arConfig = [])
    {
    ...
    static function formCalcRequest($profile, $account = false)
    {
        ...
        if ($timeOutCheck && (time() - $timeOutCheck) < 60 * \Ipolh\SDEK\option::get('timeoutRollback'))
            $result = ['error' => GetMessage('IPOLSDEK_DEAD_SERVER')];
        else {
            $mode = false;
            /*
            ...
            */
            ...
            self::setAuth(self::defineAuth(
                ($account) ? $account : ['COUNTRY' => self::$sdekCityCntr]
            ));
    ...
    public static function setOrderGoods($orderId = false)
    {
        if (isset($orderId) && $orderId > 0)
            ...
            $arFilter = ["FUSER_ID" => CSaleBasket::GetBasketUserID(), "ORDER_ID" => "NULL", "LID" => SITE_ID];
    ...
    public static function setShipmentGoods($shipmentID, $orderId = false)
    {
        ...
        $arOrderGoods = self::getBasketGoods(["ORDER_ID" => $orderId]);
        $arOrderGoods = self::filterShipmentGoods($shipmentID, $arOrderGoods);

        self::setGoods($arOrderGoods);
    }

    public static function filterShipmentGoods($shipmentID, $goods)
    { // фильтрует товары по наличию их в отправлении setShipmentGoods && sdekclass::getGoodsArray
        if (!self::canShipment())
            return false;
        $arGoods = [];
        $dbGoods = Bitrix\Sale\ShipmentItem::getList(['filter' => ['ORDER_DELIVERY_ID' => $shipmentID]]);
    ...
    public static function getGoodsDimensions($arOrderGoods, $arDefSetups = false, $isDef = 'ungiven')
    {
        ...
        foreach (GetModuleEvents(self::$MODULE_ID, "onBeforeDimensionsCount", true) as $arEvent)
            ExecuteModuleEventEx($arEvent, [&$arOrderGoods]);
    ...
    static function getBasketGoods($arFilter = [])
    {
        $arGoods = [];
        ...
        $dbBasketItems = CSaleBasket::GetList(
            ...
            false,
            false,
            ["ID", "PRODUCT_ID", "PRICE", "QUANTITY", 'CAN_BUY', 'DELAY', "NAME", "DIMENSIONS", "WEIGHT", "PRICE", "SET_PARENT_ID", "LID", "CURRENCY", "VAT_RATE", "MEASURE_NAME", "DETAIL_PAGE_URL"]
        );
    ...
    static function sumSizeOneGoods($xi, $yi, $zi, $qty)
    {
        $ar = [$xi, $yi, $zi];
        sort($ar);
        if ($qty <= 1) return (['X' => $ar[0], 'Y' => $ar[1], 'Z' => $ar[2]]);
        ...
        return (['X' => $x1 * $ar[0], 'Y' => $y1 * $ar[1], 'Z' => $z1 * $ar[2]]);
    }
    ...
    public static function weightPST($arGabs = false, $src = false)
    {
        ...
        if ($arGabs && $arGabs['D_L']) {
            $gabChecks = [$arGabs['D_L'], $arGabs['D_W'], $arGabs['D_H']];
            rsort($gabChecks);
        }
    ...
    static function checkNalD2P(&$arResult, &$arUserResult, $arParams)
    {
        if (
            ...
            if ($hNal || $hNOC) {
                ...
                if (!$arBesnalPaySys) $arBesnalPaySys = [];
        ...
        if ($payNal == '')
            ...
            switch ($payNal) {
                ...
                case '0.00':
                    ...
                    break;
                default:
                    $res = (floatval($payNal) <= floatval($orderPrice));
                    break;
            }
        }
        return $res;
    ...
    public static function noPVZOldTemplate(&$arResult, &$arUserResult)
    {
        if (
            ...
            if ($propAddr = \Ipolh\SDEK\option::get('pvzPicker')) {
                $checked = 1;
                $props = CSaleOrderProps::GetList([], ['CODE' => $propAddr]);

                while ($prop = $props->Fetch()) {
                    if (array_key_exists($prop['ID'], $arUserResult['ORDER_PROP'])) {
                        if (
                            strpos($arUserResult['ORDER_PROP'][$prop['ID']], '#S') === false &&
                            $checked !== 2
                        ) {
                            $checked = 0;
                        } else {
   ...
   public static function noPVZNewTemplate($entity, $values)
                ...
                if ($propAddr = \Ipolh\SDEK\option::get('pvzPicker')) {
                    ...
                    if (!empty($arPVZPropsIds)) {
                        ...
                        foreach ($orderProps['properties'] as $propVals) {
                            ...
                            if (
                                strpos($propVals['VALUE'][0], '#S') === false &&
                                $checked !== 2
                            ) {
                                    $checked = 0;
                            } else {
     ...
     ************************************************************************************ */
    static function setOrder($params = [])
    { // устанавливает данные для заказа-пустышки
        ...
        if (!$params['GOODS'])
            $params['GOODS'] = [["WEIGHT" => self::$orderWeight, 'QUANTITY' => 1]];
        if (!$params['GABS'])
    ...
    static function countDelivery($arOrder)
    {
        ...
        if (!$arOrder['CITY_TO_ID']) {
            $cityTo = CSaleLocation::getList([], ['CITY_NAME' => self::zaDEjsonit($arOrder['CITY_TO'])]);
            $arPriority = [[], []];
            while ($city = $cityTo->Fetch()) {
                $rezArr = array_intersect(
                    [$city['COUNTRY_NAME_ORIG'], $city['COUNTRY_NAME'], $city['COUNTRY_NAME_LANG']],
                    ['Russia', 'Russian Federation', GetMessage('IPOLSDEK_SYNCTY_rus'), GetMessage('IPOLSDEK_SYNCTY_rus2')]
                );
                if (!empty($rezArr))
                    $arPriority[0] [] = $city;
                else
        ...
        foreach ($arProfiles as $profileName => $profCost) {
            if ($profCost) {
                if ($currency)
                    $arProfiles[$profileName] = floatval(sdekExport::formatCurrency(['TO' => $currency, 'SUM' => $arProfiles[$profileName]]));
                else
                    $currency = 'RUB';
        ...
        if (self::$preSet) {
            ...
            if (!$sourse) {
                $arReturn = ['success' => false, 'error' => GetMessage('IPOLSDEK_DELIV_ERR_NOTRF')];
            } elseif ($sourse['RESULT'] == 'ERROR')
                $arReturn = ['success' => false, 'error' => $sourse['TEXT']];
            else
                $arReturn = ['success' => true, 'price' => $result, 'termMin' => $sourse['TERMS']['MIN'], 'termMax' => $sourse['TERMS']['MAX']];
        } else
            $arReturn = [
    ...
    static function cntDelivsConverted($arOrder)
    {
        ...
        $propertyCollection->setValuesFromPost(['PROPERTIES' => $arProps], []);
    ...
    {
        return self::getTarifList(['type' => $profile, 'mode' => $mode, 'answer' => 'array', 'fSkipCheckBlocks' => $fSkipCheckBlocks]);
    }
    ...
    static function loadComponent($arParams = [])
    {

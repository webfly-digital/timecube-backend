...
<script type='text/javascript'>
	IPOLSDEK_setups.table = {
        ...
        printer: {
            ...
            get: function(orderId, workMode, formType){
                IPOLSDEK_setups.table.printer.done = false;
                IPOLSDEK_setups.table.printer.tryCount = 0;
                IPOLSDEK_setups.table.printer.orderId = orderId;
                IPOLSDEK_setups.table.printer.workMode = workMode;
                IPOLSDEK_setups.table.printer.formType = formType;
                IPOLSDEK_setups.table.printer.messages = '';

                BX.showWait();
                IPOLSDEK_setups.table.printer.makeFile();
            },

            makeFile: function(){
                let action;
                switch(IPOLSDEK_setups.table.printer.formType) {
                ...
                IPOLSDEK_setups.ajax({
                    data: {
                        isdek_action: action,
                        data: {[IPOLSDEK_setups.table.printer.workMode]: [IPOLSDEK_setups.table.printer.orderId]}
                    },
                    dataType: 'json',
                    success: IPOLSDEK_setups.table.printer.onRequestFile
                });
            },
            ...
            onRequestFile: function(data){
                if (data.success === true) {
                    ...
                    if (typeof(data.notReady) !== 'undefined') {
                        if (++IPOLSDEK_setups.table.printer.tryCount > IPOLSDEK_setups.table.printer.tryMax) {
                            IPOLSDEK_setups.table.printer.done = true;
                            IPOLSDEK_setups.table.printer.messages += "<br><br>" + '<?=GetMessage('IPOLSDEK_JS_PR_MESS_MAXTRY')?>';
                        } else {
                            // Wait a bit and try again
                            setTimeout(IPOLSDEK_setups.table.printer.getFile, 300, data.notReady);
                        }
                    } else {
                        IPOLSDEK_setups.table.printer.done = true;
                    }
                    ...
                    if (data.errors.length) {
                        IPOLSDEK_setups.table.printer.messages += "<br><br>" + data.errors;
                    }
                }

                if (data.warnings.length) {
                    IPOLSDEK_setups.table.printer.messages += "<br><br>" + data.warnings;
                }

                if (IPOLSDEK_setups.table.printer.done) {
                    IPOLSDEK_setups.table.printer.finish();
                }
            },
            ...
            finish: function(){
                ...
                if (IPOLSDEK_setups.table.printer.messages.length) {
                    let dialog = new BX.CDialog({
                        title: '<?=GetMessage('IPOLSDEK_JS_PR_WND_TITLE')?>',
                        content: '<?=GetMessage('IPOLSDEK_JS_PR_MESS_ERRORS')?>' + IPOLSDEK_setups.table.printer.messages,
                        icon: 'head-block',
                        resizable: true,
                        draggable: true,
                        height: '250',
		...
		checkState: function(dispNumber){
			IPOLSDEK_setups.ajax({
				data: {
                    isdek_action: 'getOrderStateRequest',

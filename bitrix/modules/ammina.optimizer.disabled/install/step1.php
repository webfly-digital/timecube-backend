	...
	$(document).ready(function () {
		$("#am-install-mod").click(function () {
			...
			if (fieldName.val().length < 1) {
				bIsError = true;
				amInstModShowError(fieldName, '<?=Loc::getMessage("ammina.optimizer_INSTALLFORM_ERROR_NAME")?>');
			}
			amInstModResetError(fieldEmail);
			var regTest = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
			if (fieldEmail.val().length < 1) {
				bIsError = true;
				amInstModShowError(fieldEmail, '<?=Loc::getMessage("ammina.optimizer_INSTALLFORM_ERROR_EMAIL1")?>');
			} else if (!regTest.test(fieldEmail.val())) {
				bIsError = true;
				amInstModShowError(fieldEmail, '<?=Loc::getMessage("ammina.optimizer_INSTALLFORM_ERROR_EMAIL2")?>');
			}
			amInstModResetError(fieldPhone);
			if (fieldPhone.val().length < 9) {
				bIsError = true;
				amInstModShowError(fieldPhone, '<?=Loc::getMessage("ammina.optimizer_INSTALLFORM_ERROR_PHONE")?>');
			}
			if (!bIsError) {
				form.submit();
			}
		...
		<div class="ammina-modreg-wrapper">
			<div class="ammina-modreg-left">
				<div class="ammina-modreg-left-text">
					<?= Loc::getMessage("ammina.optimizer_INSTALLFORM_TITLE1") ?>
				</div>
				<div class="ammina-modreg-left-block">
					<label for="AFIELDS_NAME"><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_FIELD_NAME") ?>:</label>
					<input type="text" name="AFIELDS[NAME]" id="AFIELDS_NAME" value="<?= $USER->GetFullName() ?>"
						   placeholder="<?= Loc::getMessage("ammina.optimizer_INSTALLFORM_FIELD_NAME_PLACEHOLDER") ?>"/>
				</div>
				<div class="ammina-modreg-left-block">
					<label for="AFIELDS_EMAIL"><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_FIELD_EMAIL") ?>
						:</label>
					<input type="text" name="AFIELDS[EMAIL]" id="AFIELDS_EMAIL" value="<?= $USER->GetEmail() ?>"
						   placeholder="mail@example.ru"/>
				</div>
				<div class="ammina-modreg-left-block">
					<label for="AFIELDS_PHONE"><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_FIELD_PHONE") ?>
						:</label>
					<input type="text" name="AFIELDS[PHONE]" id="AFIELDS_PHONE" value=""
						   placeholder="+7 (495) 111-11-11"/>
				</div>
				...
				<div class="ammina-modreg-left-text2">
					<?= Loc::getMessage("ammina.optimizer_INSTALLFORM_TITLE2") ?>
				</div>
			</div>
		</div>
		<div class="ammina-modreg-right">
			<div class="ammina-modreg-right-content">
				<div class="ammina-modreg-right-text">
					<?= Loc::getMessage("ammina.optimizer_INSTALLFORM_MANAGER") ?>
				</div>
				<ul class="ammina-modreg-right-list">
					<li><span><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_MANAGER1") ?></span></li>
					<li><span><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_MANAGER2") ?></span></li>
					<li><span><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_MANAGER3") ?></span></li>
					<li><span><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_MANAGER4") ?></span></li>
					<li><span><?= Loc::getMessage("ammina.optimizer_INSTALLFORM_MANAGER5") ?></span></li>
				</ul>
			</div>
		</div>
	</div>

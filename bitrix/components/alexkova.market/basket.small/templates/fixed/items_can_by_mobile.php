...
if (!empty($arResult["BASKET_ITEMS"]["CAN_BUY"]) && count($arResult["BASKET_ITEMS"]["CAN_BUY"]) > 0) {
    ?>
    <div class="basket-body-table">
        <?php foreach ($arResult["BASKET_ITEMS"]["CAN_BUY"] as $arBasketItem):
                    ...
                    <tr>
                        <td class="basket-line-qty xs-hide sm-hide">
                            <div class="bxr-basket-group">
                                <input type="button" class="bxr-quantity-button-minus" value="-" data-item="<?= $arBasketItem["ID"] ?>" data-ratio="<?= $arBasketItem["RATIO"] ?>"
                                       data-operation="auto_save" title="<?= GetMessage("SALE_QUANTITY_MINUS") ?>">
                                <input type="text" value="<?= $arBasketItem["QUANTITY"] ?>" class="bxr-quantity-text" name="quantity" data-item="<?= $arBasketItem["ID"] ?>">
                                <input type="button" class="bxr-quantity-button-plus" value="+" data-item="<?= $arBasketItem["ID"] ?>" data-ratio="<?= $arBasketItem["RATIO"] ?>"
                                       data-operation="auto_save" data-max="<?= $arBasketItem["QTY_MAX"] ?>" title="<?= GetMessage("SALE_QUANTITY_PLUS") ?>">
                            </div>
    ...
    <div class="basket-body-title">
        ...
        <div class="pull-right">
            ...
            <a href="<?= $arParams["PATH_TO_BASKET"] ?>" class="bxr-color-button bxr-color-button-small bxr-add-order"
               <?php if ($arResult["SUMM"] < $arResult["MIN_ORDER_PRICE"]) { ?>style="display: none;"<?php } ?>>

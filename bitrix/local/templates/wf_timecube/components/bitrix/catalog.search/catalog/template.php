...
if (Loader::includeModule('search'))
{
    ...
    $searchPageParams = [
        ...
        "arrFILTER" => ["iblock_".$arParams["IBLOCK_TYPE"], "iblock_".$arSKU["IBLOCK_TYPE"]],
    ...
    // https://dev.1c-bitrix.ru/community/webdev/user/172310/blog/12201/
   $arSKU = CCatalogSKU::GetInfoByProductIBlock($arParams["IBLOCK_ID"]);
    if (!empty($arSKU)) {
        $rsIBlock = CIBlock::GetByID($arSKU["IBLOCK_ID"]);
        if ($arIBlock = $rsIBlock->fetch()) {
            $arSKU["IBLOCK_TYPE"] = $arIBlock["IBLOCK_TYPE_ID"];
        }

        if ($arParams['IBLOCK_TYPE'] != $arSKU['IBLOCK_TYPE']) {
            $searchPageParams["arrFILTER"] = ["iblock_".$arParams["IBLOCK_TYPE"], "iblock_".$arSKU["IBLOCK_TYPE"]];
            $searchPageParams["arrFILTER_iblock_".$arSKU["IBLOCK_TYPE"]] = [$arSKU['IBLOCK_ID']];
        } else {
            $searchPageParams["arrFILTER"] = ["iblock_".$arParams["IBLOCK_TYPE"]];
            $searchPageParams["arrFILTER_iblock_".$arParams["IBLOCK_TYPE"]] = [$arParams["IBLOCK_ID"], $arSKU['IBLOCK_ID']];
        }
    }
	...
	if (!empty($arElements) && is_array($arElements))
	{
        if (empty($arSKU)) {
            ...
            $searchFilter = [[
                "LOGIC" => "OR",
                ["=ID" => $arElements],
                ["=ID" => CIBlockElement::SubQuery("PROPERTY_CML2_LINK", ["IBLOCK_ID" => $arSKU['IBLOCK_ID'], "=ID" => $arElements])]
            ]];
        }

...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            this.orderData.order = this.result;
        ...
        clickNextAction: function (event) {
            ...
            if (
                ...
                titleNode = section.next.querySelector('.bx-soa-section-title-container:not(.collapse-mode)');
            ...
            if (actionSection.id === 'bx-soa-basket') {
                var collapseLink = actionSection.querySelector('.collapse-section__link');
                if (collapseLink)
                    collapseLink.classList.remove('expanded');
            }
        ...
        clickPrevAction: function (event) {
            ...
            if (section.next.id === 'bx-soa-basket') {
                var collapseLink = section.next.querySelector('.collapse-section__link');
                if (collapseLink)
                    collapseLink.classList.add('expanded');
            }
        ...
        markSectionAsCompleted: function (section) {
            ...
            if (
                ...
                titleNode = section.querySelector('.bx-soa-section-title-container:not(.collapse-mode)');
                BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
            }
        ...
        getBlockFooter: function (node) {
                ...
                buttons.push(
                    BX.create('button', {
                        props: {
                            ...
                            className: 'btn btn-xs btn-third'
                        },
            ...
            if (!isLastNode) {
                buttons.push(
                    BX.create('button', {
                        props: {href: 'javascript:void(0)', className: 'btn btn-xs btn-primary'},
        ...
        collapseSection: function () {
            var collapsingSections = document.querySelectorAll('.bx-soa-section.collapse-section');
            for (var i = 0; i < collapsingSections.length; i++) {
                this.initCollapsingSection(collapsingSections[i]);
            }
        },
        /*Инициализация сворачивающейся/разворачивающейся секции*/
        initCollapsingSection: function (sect) {
            var link = sect.querySelector('.collapse-section__link'),
                targetNode = sect.querySelector('.bx-soa-section-content');
            var soaBasket = document.getElementById('bx-soa-basket');
            var toggleSection = function (ev) {
                if (typeof ev !== "undefined") {
                    targetNode = ev.currentTarget.parentNode.parentNode.parentNode.querySelector('.bx-soa-section-content');
                    ev.preventDefault();
                }
                var showClass = 'show',
                    expandedClass = 'expanded',
                    firstClass = showClass,
                    secondClass = expandedClass,
                    timeout = 50;
                if (targetNode) {
                    if (targetNode.classList.contains(showClass)) {
                        firstClass = expandedClass;
                        secondClass = showClass;
                        timeout = 175;
                    }
                    targetNode.classList.toggle(firstClass);
                    setTimeout(function () {
                        targetNode.classList.toggle(secondClass);

                        if (targetNode.classList.contains(expandedClass))
                            link.classList.add(expandedClass);
                        else
                            link.classList.remove(expandedClass);
            ...
            /*Если секция не completed, то изначально раскрываем её*/
            if (!sect.classList.contains('bx-step-completed'))
                toggleSection();
            ...
            if (altProperty) {
                ...
                input = BX.create('INPUT', {
                    props: {
                        ...
                        autocomplete: 'city',
                        className: 'form-control input-sm bx-soa-customer-input bx-ios-fix',
        ...
        createPaySystemItem: function (item) {
            var checked = item.CHECKED == 'Y', disabled = item.DISABLED == 'Y',
                logotype, logoNode,
            ...
            if (disabled) {
                itemNode = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company col-6 disabled'},
        ...
        getDeliverySortedArray: function (objDelivery) {
            var deliveries = [],
                ...
                sortFunc = function (a, b) {
                    var sort = parseInt(a.PRICE) - parseInt(b.PRICE);
                    if (sort === 0) {
            ...
            if (this.firstLoad) {//на загрузку, делаем чек самой дешёвой доставке
                for (k in deliveries) {
                    if (deliveries.hasOwnProperty(k)) {
                        if (deliveries[k].CHECKED == 'Y') {
                            delete deliveries[k].CHECKED;
                        }
                    }
                }
                deliveries[0].CHECKED = 'Y';
            }
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            ...
            if (property.getId() == 7) { //если проперти Адрес доставки
                this.result.DELIVERY.forEach((element) => {
                    if (element.ID == 18 || element.ID == 3 || element.ID == 24 || element.ID == 19) { //если выбрана служба доставки Самовывоз
                        if (element.CHECKED == 'Y') {
                            if (property.getValue() == '') {
                                property.setValue('-'); //заполняем Свойство адрес доставки
                            }
                            propsItemNode.style.display = 'none';//скрываем  Свойство адрес доставки
                        }
                    }
                })
            }
        ...
        alterProperty: function (settings, propContainer) {
            ...
            for (i = 0; i < inputs.length; i++) {
                ...
                BX.addClass(inputs[i], 'form-control input-sm bx-soa-customer-input bx-ios-fix');
            }
            ...
            if (add.length) {
                ...
                BX.bind(add, 'click', BX.delegate(function (e) {
                    ...
                    if (textInputs.length) {
                        ...
                        BX.addClass(textInputs[textInputs.length - 1], 'form-control input-sm bx-soa-customer-input bx-ios-fix');
            ...
            for (i in this.result.GRID.ROWS) {
                if (this.result.GRID.ROWS.hasOwnProperty(i)) {
                    ...
                    products.push({
                        ...
                        'name': item.data.NAME + ' ' + item.data['PROPERTY_CML2_ARTICLE_VALUE'],
                        'price': item.data.PRICE,
                        'category': item.data['PROPERTY_XXX_GOOGLE_PRODUCT_CATEGORY_VALUE'], // для отображения категорий в расширеной торговли гугл аналитики
                        ...
                        'ecommerce': {
                            ...
                            'purchase': {
                                'actionField': {
                                    ...
                                    'coupon': this.result.COUPON_LIST.length != 0 ? this.result.COUPON_LIST[0].COUPON || [] : '',
                                },
                                'products': products
                            }
                        }

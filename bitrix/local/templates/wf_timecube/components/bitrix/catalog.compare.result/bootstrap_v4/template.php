...
<div class="catalog-compare mb-4 bx-<? echo $templateData['TEMPLATE_CLASS']; ?>" id="bx_catalog_compare_block"><?
	...
	<div class="mb-3 d-flex align-items-center">
	    <ul class="nav-simple">
	        <li><span class="nav-simple__caption"><?= GetMessage("CATALOG_SHOWN_CHARACTERISTICS") ?>:</span></li>
	        <li class="<? echo (!$arResult["DIFFERENT"] ? 'active' : ''); ?>"><a class=""
                				href="<? echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=N'; ?>"
                				rel="nofollow"><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?></a>
                </li>
	        <li class="<? echo ($arResult["DIFFERENT"] ? 'active' : ''); ?>">
	        <a class=""
							...
							<? foreach($arResult["ITEMS"] as $arElement)
							{
								...
								<td>
									<? switch($code)
									{
										case "NAME":
											?>
											<div class="catalog-compare-item-heading">
											<a class="catalog-compare-item-title" href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=$arElement[$code]?></a>
											<? if($arElement["CAN_BUY"]): ?>
												<div class="pt-2">
												    <div class="btn-animated-wrapper">
													<a class="btn btn-third btn-sm btn-animated bnt-cart-simple" href="<?=$arElement["BUY_URL"]?>" rel="nofollow"><span class="svg-icon icon-cart"></span></a>
												    </div>
												</div>
										...
										case "DETAIL_PICTURE":
											if (!empty($arElement["FIELDS"][$code]) && is_array($arElement["FIELDS"][$code])):?>
												<a class="pic-ratio pic-ratio-6by5" href="<?=$arElement["DETAIL_PAGE_URL"]?>">
													<img
														border="0"
						...
						<tr>
							...
							<?foreach($arResult["ITEMS"] as $arElement)
							{
								...
								<td>
									<?
//                                    if ($code == 'MANUFACTUR') {
//                                        echo $arElement["DISPLAY_PROPERTIES"][$code]["VALUE"];
//                                    } else {
                                        $val = (is_array($arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"])? implode("/ ", $arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]): $arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]);
                                        echo empty($val) ? '–' : $val;
//                                    }
                                    ?>
								</td>

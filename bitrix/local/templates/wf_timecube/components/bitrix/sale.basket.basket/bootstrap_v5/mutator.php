...
foreach ($this->basketItems as $row)
{
    ...
    $idSKU = CCatalogSku::GetProductInfo($row['PRODUCT_ID']);
    if(is_array($idSKU)){
        $row["PRODUCT_ID"] = $idSKU['ID'];
    }

    $prod = CIBlockElement::GetList([],
        ['IBLOCK_ID' => WF_CATALOG_IBLOCK_ID,'=ID'=>$row["PRODUCT_ID"], '!SECTION_CODE' => WF_PACK_SECTION_CODE], false,false,
        ['IBLOCK_ID','PROPERTY_IS_PACK_FREE','IBLOCK_SECTION_ID', 'PROPERTY_MANUFACTUR', 'PROPERTY_ARTNUMBER', 'PROPERTY_XXX_GOOGLE_PRODUCT_CATEGORY'])->fetch();
    $mxResult = CCatalogSku::GetProductInfo(
        $row['PRODUCT_ID']
    );
    if ($prod) {
        if ($prod["IBLOCK_ID"] == WF_CATALOG_IBLOCK_ID)
            $row['SHOW_PACK_BTN'] = true;
        if ($prod["PROPERTY_IS_PACK_FREE_VALUE"] == 'Да')
            $row['FREE_PACK'] = true;
    }

    $rowData = [
        'SHOW_PACK_BTN' => $row['SHOW_PACK_BTN'],// '<span class="giftcard-alt"></span> '
        'PACK_BTN_TITLE' => ($row['FREE_PACK'] ? 'Выбрать подарочную упаковку бесплатно' :'Выбрать подарочную упаковку'),
        'FREE_PACK' => $row['FREE_PACK'],
        'ID' => $row['ID'],
        ...
        'SHOW_LABEL' => false,
        'LABEL_VALUES' => [],
        'BRAND' => $prod["PROPERTY_MANUFACTUR_VALUE"], //добираем свойства для гугл аналитики
        'CATEGORY' => $prod["PROPERTY_XXX_GOOGLE_PRODUCT_CATEGORY_VALUE"],
        'ARTNUMBER' => $prod["PROPERTY_ARTNUMBER_VALUE"],
    ];

    if ($row['XML_ID'] == 'PACK') $rowData['NO_CONTROLS'] = true;
    if ($row['XML_ID'] == 'PEN') {
        $rowData['NO_CONTROLS'] = true;
        $row['PREVIEW_PICTURE_SRC'] = $row['DETAIL_PICTURE_SRC'] = '/assets/img/pen.jpg';
    }
    if ($row['XML_ID'] == 'MASK') {
        $rowData['NO_CONTROLS'] = true;
        $row['PREVIEW_PICTURE_SRC'] = $row['DETAIL_PICTURE_SRC'] = '/assets/img/10_masks.png';
    }
    if ($row['XML_ID'] == 'MOET') {
        $rowData['NO_CONTROLS'] = true;
        $row['PREVIEW_PICTURE_SRC'] = $row['DETAIL_PICTURE_SRC'] = '/assets/img/moet.jpg';
    }

...
class WFBuyOneClick extends CBitrixComponent implements Controllerable
{
    ...
    public function buyOneClickAction($pid, $name, $email, $phone, $msg, $action = false, $token = false)
    {

        AddMessage2Log($action);
        AddMessage2Log($_SERVER['REMOTE_ADDR']);
        if ($action == 'buy_one_click') {
            $privatekey = '6LcO9B8qAAAAANtfuIuR8TIhz-Y35eBiS7sIQK8Q';
            $ch = curl_init();
            ...
            AddMessage2Log($resp);
            curl_close($ch);
            if ($resp['success'] == false && floatval($resp['score']) < 0.5) {
...
//        ]);
    ...
    {
        $this->arParams['CAPTCHA'] = 'N';
        $res = \CIBlockElement::GetList([], ['IBLOCK_CODE' => WF_SETTINGS_IBLOCK_CODE, 'CODE' => SITE_SERVER_NAME, 'PROPERTY_CAPCHA_BUY_ONE_CLICK_VALUE' => 'Y'], false, false, ['ID', 'PROPERTY_CAPCHA_BUY_ONE_CLICK']);
        while ($arElement = $res->Fetch()) {
            $this->arParams['CAPTCHA'] = 'Y';
        }

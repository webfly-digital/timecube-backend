...
if (isset($arResult['ITEM']))
{
	...
	$showSlider = false;//is_array($morePhoto) && count($morePhoto) > 1;
	...
	$bьфuttonSizeClass = isset($arResult['BIG_BUTTONS']) && $arResult['BIG_BUTTONS'] === 'Y' ? 'btn-md' : 'btn-sm';
	$itemHasDetailUrl = isset($item['DETAIL_PAGE_URL']) && $item['DETAIL_PAGE_URL'] != '';

	//endregion
	?>
    <?
        $innerClass = 'product-card__inner'; //для любых карточек
        if(strtolower($arResult['TYPE']) == 'row')
            $innerClass = 'product-row__inner'; //только для строчного отображения
    ?>
    <div class="<?=$innerClass?>" id="<?=$areaId?>" data-entity="item">
        <?
		...
		if (!$haveOffers)
		{
			$jsParams = array(
				...
				'PRODUCT' => array(
					'ID' => $item['ID'],
					'NAME' => $productTitle,
					'DETAIL_PAGE_URL' => $item['DETAIL_PAGE_URL'],//$item['DETAIL_PAGE_URL'],
					'PICT' => $item['PREVIEW_PICTURE'],//$item['SECOND_PICT'] ? $item['PREVIEW_PICTURE_SECOND'] : $item['PREVIEW_PICTURE'],
		...
		$jsParams['BRAND_PROPERTY'] = $item['PROPERTIES'][$arParams['BRAND_PROPERTY']]['VALUE'];/*!empty($item['DISPLAY_PROPERTIES'][$arParams['BRAND_PROPERTY']])
			? $item['DISPLAY_PROPERTIES'][$arParams['BRAND_PROPERTY']]['DISPLAY_VALUE']
			: null;*/
        $jsParams['CATEGORY'] = $item['PROPERTIES']['XXX_GOOGLE_PRODUCT_CATEGORY']['VALUE']; // добираем для гугл аналитики

...
<?    if ($item['PROPERTIES']['SHOW_YOUTUBE']["VALUE_XML_ID"] == "74871818d9ae45163be185c1318f2909") {
        ...
        <?    }?>
<div class="product-row__visual" data-entity="image-wrapper">
    <a class="pic" href="<?= $item['DETAIL_PAGE_URL'] ?>">
        <? if (!empty($item['HOVER_PHOTO'])) { ?>
            <img id="<?= $itemIds['PICT'] ?>" title="<?= $imgTitle ?>" class="lozad main"
                 data-src="<?= $item['PREVIEW_PICTURE']['SRC'] ?>"/>
            <img class="lozad hover" data-src="<?= $item['HOVER_PHOTO'] ?>"/>
        <? } else { ?>
            <img id="<?= $itemIds['PICT'] ?>" title="<?= $imgTitle ?>" class="lozad"
                 data-src="<?= $item['PREVIEW_PICTURE']['SRC'] ?>">
        <? } ?>
    </a>
    <? if (!$actualItem['CAN_BUY']): ?>
    <? else: ?>
        <div class="product-row__labels d-md-none">
            <div class="product-labels product-labels--column">
                <?
                include 'labels.php';
                ?>
            </div>
...
<div class="product-row__details">
    <div class="product-row__details-left">
        <!--заголовок-->
        <div class="product-row__line">
            <ul class="product-row__list">
                <li><span class="product-card__brand">Производитель: <?= $arResult['MANUFACTUR']['NAME'] ?></span></li>
                <? if ($arResult['MANUFACTUR']['FLAG_URL']['VALUE'] != 'china') { ?>
                    <li>
                  <span class="product-card__country reverse">
                      <span><?= $arResult['MANUFACTUR']['COUNTRY_RU']['VALUE'] ?></span>
            ...
            <p class="product-row__title"><a href="<?= $item['DETAIL_PAGE_URL'] ?>"><?= $productTitle ?></a></p>
        </div>
        <!--конец заголовка-->
        ...
        $displayOffers = $arParams['PRODUCT_DISPLAY_MODE'] === 'Y' && $haveOffers && !empty($item['OFFERS_PROP']); ?>
        <div class="product-row__line <?= $displayOffers ? '' : 'd-none' ?>">
            <?
            if ($displayOffers) {
                ?>
                <div class="product-card__row product-item-info-container product-item-hidden"
                     id="<?= $itemIds['PROP_DIV'] ?>">
                    <?
                    foreach ($arParams['SKU_PROPS'] as $skuProperty) {
                        ...
                        <div data-entity="sku-block" class="product-card__scu-block">
                            <div class="product-item-scu-container" data-entity="sku-line-block">
                                <div class="product-item-scu-block-title product-card__scu-title"><?= $skuProperty['NAME'] ?>
                                    :
                                </div>
        ...
        ?><? if (!$actualItem['CAN_BUY']): ?>
        <? else: ?>
            <div class="product-row__line d-none d-md-block">
    ...
    <div class="product-row__details-right">
        <?

        // region price
        ?>
        <div class="product-row__line" data-entity="price-block">
            <div class="product-card__prices <?= ($price['RATIO_PRICE'] >= $price['RATIO_BASE_PRICE']) || !$actualItem['CAN_BUY'] ? '' : 'product-card__prices--discount' ?>">
                <p class="caption-gray"><?= (!$actualItem['CAN_BUY']) ? '' : 'Цена:' ?></p>
...
//                                    '#UNIT#' => $minOffer['ITEM_MEASURE']['TITLE']
                            ...
                            echo (!$actualItem['CAN_BUY']) ? $arParams['MESS_NOT_AVAILABLE'] : $price['PRINT_RATIO_PRICE'];
                        }
                    } else {
                ...
                if (!$haveOffers) {
                    if ($actualItem['CAN_BUY']) {
                        ...
                        <div class="product-item-button-container  btn-animated-wrapper">
                            ...
                            ?>
                            <button class="btn btn-link <?= $buttonSizeClass ?>" style="display: none"
                    ...
                    if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y') {
                        ...
                        ?>
                        <div class="product-card__button-wrapper btn-animated-wrapper wide">
                            <a class="btn btn-sm btn-third btn-animated btn-cart"
                    ...
                    <div class="product-row__line">
                        <ul class="product-row__list">
                            <li
                                    ...
                                    title="<?= $arParams['MESS_SHOW_MAX_QUANTITY'] ?>">
                                <div class="availability availability--sm">
                            <span class="availability__scale">
                                <svg class="sprite-icon">
                                  <use xlink:href="/assets/img/sprite.svg#a-<?= $svgParam ?>"></use>

...
if ($USER->isAdmin()) {
    echo "<pre>";
    echo "[ADMIN] Примененные скидки:";
    foreach ($arResult['APPLIED_DISCOUNT_LIST'] as $d) {
        echo "<br>";
        echo ($d['NAME']);
    }
    echo "</pre>";
...
if (empty($arResult['ERROR_MESSAGE'])) {
    ...
    <div id="basket-root" class="bx-basket bx-<?= $arParams['TEMPLATE_THEME'] ?> bx-step-opacity" style="opacity: 0;">
        ...
        <?
        if (
                $arParams['BASKET_WITH_ORDER_INTEGRATION'] !== 'Y' && in_array('bottom', $arParams['TOTAL_BLOCK_DISPLAY'])
        ) {
            ?>
            <div class="row">
                <div class="col" data-entity="basket-total-block"></div>

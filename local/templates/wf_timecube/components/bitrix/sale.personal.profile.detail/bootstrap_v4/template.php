	...
	<div class="row mb-3">
		<form method="post"  class="col sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
			...
			<div class="row mb-2">
				...
				<div class="col">
					<h4 class="text-center"><?= Loc::getMessage('SPPD_PROFILE_NO', array("#ID#" => $arResult["ID"]))?></h4>
				</div>
			</div>

			<div class="form-group mb-2">
				<label class="col-form-label"><?=Loc::getMessage('SALE_PERS_TYPE')?>:</label>
				<div class=""><?=$arResult["PERSON_TYPE"]["NAME"]?></div>
			</div>

			<div class="form-group mb-2">
				<label class="col-form-label" for="sale-personal-profile-detail-name"><?=Loc::getMessage('SALE_PNAME')?>:<span class="req">*</span></label>
			...
			foreach($arResult["ORDER_PROPS"] as $block)
			{
				if (!empty($block["PROPS"]))
				{
					...
					<div class="row mb-2 mt-4">
						<div class="col">
							<h4 class="text-center"><?= $block["NAME"]?></h4>
						</div>
					</div>
					<?
					foreach($block["PROPS"] as $property)
					{
						...
						?>
							...
							if ($property["TYPE"] == "CHECKBOX")
							{
								...
								<div class="form-group mb-2">
                                    <div class="checkbox">
                                        <label class="col-form-label <?=$alignTop?>" for="sppd-property-<?=$key?>">
							...
							{
								?>
								<p class="caption"><?= $property["DESCRIPTION"] ?></p>
								<?
							}
							?>
						<?
			...
			<div class="form-footer">
                ...
                <input type="submit" class="btn btn-third btn-md"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
                <input type="submit" class="btn btn-third btn-md"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
            </div>
		</form>
	</div>
	<?
